...
class ShippingEstimator
{
    ...
    public function estimateShipping(CartInterface|Quote $quote): ?Total
    {
        $shippingAddress = $this->getShippingAddress($quote);
        return $this->processShippingAssignment($quote, $shippingAddress);
    }

    private function getShippingAddress(CartInterface|Quote $quote)
    {
        if ($quote->getCustomerId(
            ) && $shippingAddress = $this->getCustomerShippingAddress($quote)) {
    ...
    private function getCustomerShippingAddress(CartInterface|Quote $quote)
    {
        $shippingAddress = $quote->getShippingAddress();
        ...
        $addressByCustomer = $quote->getShippingAddressByCustomerAddressId(
        ...
        usort(
            ...
            function ($method1, $method2) {
                if ($this->config->getUseLowest()) {
                    return $method1->getPriceInclTax(
                        ) <=> $method2->getPriceInclTax();
                }
                return $method2->getPriceInclTax(
                    ) <=> $method1->getPriceInclTax();
            }
        );
        return reset($methods);
    }
